...
class DC2_SEDBuilder(SEDBuilder):
    ...
    def buildSED(self, config, base, logger):
        ...
        sed_bins_array = np.take_along_axis(sed_bins_array * np.asarray([1+1e-9, 1-1e-9]), _idx, axis=0).ravel()  # narrow the bins by a small factor to avoid overlap
